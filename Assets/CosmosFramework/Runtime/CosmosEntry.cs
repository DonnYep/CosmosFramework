...
namespace Cosmos
{
    public class CosmosEntry 
    {
        ...
        public static void LaunchAppDomainHelpers()
        {
            ...
            {
                var helper = Utility.Assembly.GetInstanceByAttribute<ImplementerAttribute, IDebugHelper>(assemblies[i]);
                if (helper != null)
                {
            ...
            for (int i = 0; i < length; i++)
            {
                var helper = Utility.Assembly.GetInstanceByAttribute<ImplementerAttribute, IJsonHelper>(assemblies[i]);
                if (helper != null)
                {
            ...
            for (int i = 0; i < length; i++)
            {
                var helper = Utility.Assembly.GetInstanceByAttribute<ImplementerAttribute, IMessagePackHelper>(assemblies[i]);
                if (helper != null)
                {
        ...
        {
            var debugHelper = Utility.Assembly.GetInstanceByAttribute<ImplementerAttribute, IDebugHelper>(assembly);
            if (debugHelper!= null)
            {
                Utility.Debug.SetHelper(debugHelper);
            }
            var jsonHelper = Utility.Assembly.GetInstanceByAttribute<ImplementerAttribute, IJsonHelper>(assembly);
            if (jsonHelper!= null)
            {
                Utility.Json.SetHelper(jsonHelper);
            }
            var mpHelper = Utility.Assembly.GetInstanceByAttribute<ImplementerAttribute, IMessagePackHelper>(assembly);
            if (mpHelper != null)
            {
