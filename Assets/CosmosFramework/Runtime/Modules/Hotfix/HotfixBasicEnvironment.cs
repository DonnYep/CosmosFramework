...
namespace Cosmos
{
    ...
    public abstract class HotfixBasicEnvironment
    {
        ...
        public HotfixBasicEnvironment(Assembly hotfixAssembly)
        {
            ...
            {
                throw new ArgumentNullException("Hotfix-->>进入热更新流程失败：未指定热更新入口流程！");
            }
            GameManager.GetModule<IHotfixManager>().UpdateHotfixLogicEvent += UpdateHotfixLogic;
        }
        /// <summary>
        /// 更新逻辑
        /// </summary>
        ...
        public void SwitchProcedure<T>() where T : HotfixProcedureBase
        {
            ...
            {
                throw new ArgumentNullException($"Hotfix->>切换热更新流程失败：不存在流程 {typeof(T).Name } 或者流程未激活！");
            }
        }
    }
}