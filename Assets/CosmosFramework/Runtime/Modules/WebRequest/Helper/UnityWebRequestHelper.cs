...
namespace Cosmos
{
    ...
    {
        public static Coroutine DownloadTextAsync(string uri, WebRequestCallback webRequestCallback)
        {
            return Utility.Unity.StartCoroutine(EnumUnityWebRequest(UnityWebRequest.Get(uri),webRequestCallback));
        }
        ...
        static IEnumerator EnumUnityWebRequest(UnityWebRequest unityWebRequest, WebRequestCallback webRequestCallback)
        {
            using (UnityWebRequest request = unityWebRequest)
            {
                request.SendWebRequest();
                var args = WebRequestUpdateEventArgs.Create(unityWebRequest.url, null);
                while (!request.isDone)
                {
                    WebRequestUpdateEventArgs.Update(request.downloadProgress, args);
                    webRequestCallback.UpdateCallback?.Invoke(null, args);
                    yield return null;
                }
                if (!request.isNetworkError && !request.isHttpError)
                {
                    if (request.isDone)
                    {
                        WebRequestUpdateEventArgs.Update(1, args);
                        webRequestCallback.UpdateCallback?.Invoke(null, args);
                        var successEventArgs = WebRequestSuccessEventArgs.Create(request.url, request.downloadHandler.data);
                        webRequestCallback.SuccessCallback?.Invoke(null, successEventArgs);
                    }
                }
                else
                {
                    var failureEventArgs = WebRequestFailureEventArgs.Create(request.url, request.error);
                    webRequestCallback.FailureCallback?.Invoke(null, failureEventArgs);
                }
            }
        }
    }
