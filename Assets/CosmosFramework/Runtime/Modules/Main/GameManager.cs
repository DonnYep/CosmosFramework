...
namespace Cosmos
{
    ...
    public static class GameManager
    {
        ...
        static void ModuleTermination(Module module)
        {
            ...
            if (HasModule(type))
            {
                Utility.Assembly.InvokeMethod(module, ModuleConst.OnDeactive);
                ...
                moduleCount--;
                Utility.Assembly.InvokeMethod(module, ModuleConst.OnTermination);
                    ...
                    if (typeof(IModuleManager).IsAssignableFrom(type))
                    {
                        if (!HasModule(type))
                        {
                            if (moduleDict.TryAdd(type, modules[i]))
                            {
                                try
                                {
                                    Utility.Assembly.InvokeMethod(modules[i], ModuleConst.OnInitialization);
                                    moduleCount++;
                                }
                                catch (Exception e)
                                {
        ...
        static void ActiveModule()
        {
            foreach (var module in moduleDict.Values)
            {
                try
                {
                    Utility.Assembly.InvokeMethod((module as Module), ModuleConst.OnActive);
                }
                catch (Exception e)
                {
        ...
        static void PrepareModule()
        {
            foreach (var module in moduleDict.Values)
            {
                try
                {
                    Utility.Assembly.InvokeMethod(module , ModuleConst.OnPreparatory);
