...
namespace Cosmos
{
    public interface IUIManager : IModuleManager
    {
        ...
        Coroutine OpenUIFormAsync<T>(Action<T> loadDoneCallback = null) where T : UIForm;
        /// <summary>
        ...
        /// <returns>协程对象</returns>
        Coroutine OpenUIFormAsync(UIAssetInfo assetInfo, Type uiType, Action<UIForm> loadDoneCallback = null);
        /// <summary>
        /// 通过UIAssetInfo加载UI对象
        /// </summary>
        ...
        /// <param name="loadDoneCallback">加载完成后的回调</param>
        /// <returns>协程对象</returns>
        Coroutine OpenUIFormAsync<T>(UIAssetInfo assetInfo, Action<T> loadDoneCallback = null) where T : UIForm;
        /// <summary>
        /// 通过特性UIAssetAttribute加载Panel（异步）；
        /// </summary>
        ...
        /// <param name="loadDoneCallback">加载成功的回调。若失败，则不执行</param>
        /// <returns>协程对象</returns>
        Coroutine OpenUIFormAsync(Type type, Action<UIForm> loadDoneCallback = null);
        ...
        T PeekUIForm<T>(string uiAssetName) where T : UIForm;
        UIForm PeekUIForm(string uiAssetName);
        void CloseUIGroup(string uiGroupName);
        void HideUIGroup(string uiGroupName);
        void ShowUIGroup(string uiGroupName);
    }
}
