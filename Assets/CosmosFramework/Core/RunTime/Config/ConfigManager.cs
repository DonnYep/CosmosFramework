...
namespace Cosmos.Config
{
    ...
    internal sealed class ConfigManager : Module<ConfigManager>
    {
        ...
        internal bool GetConfigBool(ushort cfgKey)
        {
        ...
        /// <returns>读取的布尔值。</returns>
        internal bool GetConfigBool(ushort cfgKey, bool defaultValue)
        {
            ConfigData? configData = GetConfigData(cfgKey);
        ...
        internal int GetConfigInt(ushort cfgKey)
        {
            ConfigData? configData = GetConfigData(cfgKey);
            if (!configData.HasValue)
            {
        ...
        /// <param name="defaultValue">当指定的全局配置项不存在时，返回此默认值。</param>
        /// <returns>读取的整数值。</returns>
        internal int GetConfigInt(ushort cfgKey, int defaultValue)
        {
            ConfigData? configData = GetConfigData(cfgKey);
        ...
        internal float GetConfigFloat(ushort cfgKey)
        {
            ConfigData? configData = GetConfigData(cfgKey);
            if (!configData.HasValue)
            {
        ...
        /// <param name="defaultValue">当指定的全局配置项不存在时，返回此默认值。</param>
        /// <returns>读取的浮点数值。</returns>
        internal float GetConfigFloat(ushort cfgKey, float defaultValue)
        {
            ConfigData? configData = GetConfigData(cfgKey);
        ...
        internal string GetConfigString(ushort cfgKey)
        {
            ConfigData? configData = GetConfigData(cfgKey);
            if (!configData.HasValue)
            {
        ...
        /// <param name="defaultValue">当指定的全局配置项不存在时，返回此默认值。</param>
        /// <returns>读取的字符串值。</returns>
       internal string GetConfigString(ushort cfgKey, string defaultValue)
        {
            ConfigData? configData = GetConfigData(cfgKey);
